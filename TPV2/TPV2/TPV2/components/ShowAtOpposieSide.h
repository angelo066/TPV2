...
public:
	...
	void update() override {
		if (tr_->getPos().getX() > sdlutils().width()) {
			tr_->getPos().setX(0 - tr_->getW());		//Le restamos su anchura porque si no aparece autom�ticamente en pantalla	
		}
		...
		else if(tr_->getPos().getY() < 0 - tr_->getH()){//Le restamos su altura porque si no aparece autom�ticamente en pantalla	
